...
export const sstDocumentTypes = pgTable("sst_document_types", {
  ...
  /**
   * Qué actividades del PDTP acredita **publicar** una versión de un documento
   * de este tipo. El hecho es que existe una versión vigente — la N°43, los
   ...
   * configurar uno por uno. Sigue el mismo patrón que las plantillas de
   * inspección, los cursos, las campañas y los planes de emergencia: el módulo
   * origen declara su relación con el programa anual.
   *
  ...
  /**
   * Qué actividades acredita cada **acuse de recibo**, que es un hecho
   * distinto y posterior.
   *
   * Son dos columnas y no una porque son dos momentos: publicar prueba que el
   * documento existe, acusar prueba que la gente lo recibió. La N°36 —difusión
   * de la matriz MIPER— es lo segundo y se mide por cobertura sobre los
   * destinatarios. Con una sola columna, declarar la N°36 la habría acreditado
   * **al publicar**, sin un solo acuse: con una unidad planificada al mes,
   * publicar habría saldado el mes entero de una difusión que nadie recibió.
   *
   * Es la misma separación que las plantillas de inspección ya tienen entre
