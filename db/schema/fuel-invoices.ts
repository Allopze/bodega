...
export const fuelLoads = pgTable("fuel_loads", {
  ...
  productId:      text("product_id").notNull().references(() => fuelProducts.id), // Relación canónica; product queda como snapshot legacy.
  ...
  index("fuel_loads_product_idx").on(table.productId),
...
export const fuelLoadsRelations = relations(fuelLoads, ({ one }) => ({
  ...
  fuelProduct: one(fuelProducts, { fields: [fuelLoads.productId], references: [fuelProducts.id] }),
