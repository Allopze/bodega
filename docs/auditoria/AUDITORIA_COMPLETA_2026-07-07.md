...
### [ALTO] PPA offline/PWA no conserva el flujo en modo sin conexion

**Categoria:** Usabilidad / Resiliencia  
**Estado:** Confirmado  
**Evidencia:** `npm run test:e2e` falla repetidamente en `e2e/ppa-offline.spec.ts`; el navegador queda en `chrome-error://chromewebdata/` en vez de terminar con `?saved=offline`, y tambien fallan validaciones de manifest/service worker/cache.  
**Impacto:** PPA es un flujo de seguridad operacional. Si la experiencia offline falla en terreno, el trabajador puede perder el envio o quedar sin confirmacion usable.  
**Remediacion:** Reproducir con una sola spec PPA offline, validar registro de service worker/manifest, asegurar fallback navegable y persistencia IndexedDB, y agregar aserciones de UI sin depender de navegacion de red.
