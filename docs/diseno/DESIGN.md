...
┌─────────────────────────────────────────────────────────────┐
│ bg-bg · p-3 · gap-3                                         │
│  ┌──────────────┐   superficie continua (bg-bg)             │
│  │ Sidebar card │  ┌──────────────────────────────────────┐ │
│  │ w-60         │  │ TopBar — h-[3.25rem], border-b       │ │
│  │ rounded-2xl  │  │ contexto / breadcrumb     🔔  AZ     │ │
│  │ shadow-card  │  ├──────────────────────────────────────┤ │
│  │ bg-surface   │  │ <main> — overflow-y-auto, bg-bg      │ │
│  └──────────────┘  │ max-w-[1440px], px-4 md:px-8, py-6  │ │
│                    └──────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```

El shell usa `h-[100dvh] lg:p-3` con `lg:gap-3`. El **sidebar** es el único elemento
como tarjeta flotante (`rounded-2xl shadow-card bg-surface`). El panel principal es una
**superficie continua** sin marco: comparte `bg-bg` con el lienzo exterior, sin
`rounded` ni `shadow-card`. En mobile el sidebar desaparece y el panel ocupa la pantalla.

- **TopBar**: `bg-bg border-b border-(--color-border)`. Desktop: breadcrumb de contexto
  (sección / página) derivado de `NAV_ITEMS` + `usePathname`, chip de faena opcional,
  campana y avatar a la derecha. Mobile: hamburguesa + marca + campana + avatar.
- **Sidebar**: tarjeta flotante, `w-60` colapsable a `4.75rem`, `rounded-2xl`, `shadow-card`
- **Main**: `bg-bg`, `max-w-440` (1760px), centrado, fondo `bg`
...
Estructura numerada con 4 secciones. Los números de sección son decorativos (`aria-hidden`)
para no contaminar el anuncio del screen reader:
...
03 · Bodega
    Bodega
    Entregas
04 · Reportes          ← pinned to bottom of the rail via mt-auto
    Reportes
```

Cada ítem:
- Ícono Phosphor, 15px, weight cambia con estado activo (`regular` → `bold`)
- **Activo**: `border-l-2 border-(--color-text)` — regla izquierda visible, sin fondo.
  Label en `text-(--color-primary-ink) font-semibold`. Icono en `text-(--color-primary)`.
- **Inactivo**: `border-l-2 border-transparent` — borde invisible mantiene el padding
  constante para que el texto nunca salte. `rounded-r-md` en hover para el fill lateral.
- Hover: `bg-surface-2`, transición de color 120ms
- Badge de conteo pendiente: naranja signal, mono, `99+` para >99
- Los ítems se filtran por permisos del usuario
- La sección final (Reportes) se fija al fondo del rail con `mt-auto` en el flex column
...
- `bg-bg border-b border-(--color-border)`, altura `h-[3.25rem]`
- **Desktop (izquierda):** breadcrumb de contexto (sección / página) derivado de
  `NAV_ITEMS` + `usePathname()` usando el componente `<Breadcrumbs>`. Chip de faena
  opcional junto al breadcrumb cuando aplica.
- **Desktop (derecha):** campana de notificaciones con ping animado y badge, avatar con
  dropdown (perfil, cerrar sesión, admin si aplica).
- **Mobile:** hamburguesa + marca Chome (logo + texto) a la izquierda, campana + avatar
  a la derecha. El breadcrumb de contexto está oculto en mobile (`hidden lg:flex`).
