...
**Estado:** en ejecucion; Fases 0 y 1 cerradas tecnicamente, Fase 4 cerrada tecnicamente salvo la clasificacion de negocio de 22 actividades (4.3), Fase 2 y Fase 3 avanzadas con items grandes pendientes (calendario general, multifaena, autosave completo, preview por faena), aceptacion operacional pendiente en todas las fases

**Fecha de corte de la revision:** 2026-07-21

**Documento de referencia inicial:** `PROGRAMA DE TRABAJO PREVENTIVO SG-SST 2026.xlsx`
...
Ante una contradiccion, prevalecen en este orden: intencion preventiva y decision formal de negocio, controles de seguridad/integridad, modelo general del producto, datos del XLSX vigente, codigo y esquema actuales, este plan, y finalmente documentos historicos. Ningun checklist antiguo marcado como terminado sustituye la verificacion en la base y el flujo productivo reales.
...
| Celdas semanales planificadas con cantidad | 843 |
| Cantidad planificada anual total | 1.035 |
...
| Archivo recibido | `PROGRAMA DE TRABAJO PREVENTIVO SG-SST 2026.xlsx` |
| Tamaño del archivo | 4.513.110 bytes |
| SHA-256 | `a6adc0fa017a9972dde09e5abdddb399cfce23526332807a122e52bfd15690a4` |

### 2.1 Hojas del archivo y membresias

| Clave interna esperada | Vista del archivo | Actividades |
|---|---|---:|
| `pdtp_general` | Programa general | 89 |
| `cphs` | CPHS | 4, actividades 11 a 14 |
| `prf_adm_contrato` | PRF + Administrador de contrato | 76 |
...
- `db/seed/pdtp-catalog-2026.json` coincide con el archivo vigente en objetivos, actividades, hojas, membresias y planificacion: 8/89/8, 843 celdas y total 1.035.
...
9. **XLSX es un adaptador.** La carga del legado y las salidas tabulares usan `.xlsx`, pero crear y operar un programa no depende de importar o editar archivos.
...
| SEC-01 | P0 | El exportador solo valida acceso si recibe `faena`; sin ese parametro, los planes de accion pueden consultarse sin alcance y mezclar faenas. | Exposicion transversal de datos en XLSX. | 1 |
...
| IMP-03 | P1 | La interfaz/ruta acepta `.xls`, pero la lectura usa el formato XLSX de ExcelJS. | Contrato engañoso y fallos de carga. | 3 |
...
| INT-02 | P1 | No existe una clave idempotente transversal para convertir eventos operacionales en ejecuciones PDTP. | Riesgo de doble contabilizacion o edicion manual no trazable. | 5 |
| EXP-01 | P1 | La salida actual no ofrece una vista operacional clara y sus formatos se confunden con el documento de origen. | Se intenta resolver operacion y archivo historico con un mismo XLSX. | 6 |
...
- Cada resultado autenticado —exitoso, denegado, invalido o fallido— deja auditoria con usuario, programa y faena solicitada, sin depender de que el XLSX llegue a generarse.
...
```text
cargar XLSX -> validar estructura -> extraer a staging -> mostrar diff
...
4. **Perfil de compatibilidad 2026:** salida XLSX que reconstruye la informacion necesaria del RE-36 si la organizacion aun debe intercambiar ese formato.
...
- [x] Registrar SHA-256, tamano y nombre del XLSX recibido.
...
- Existe una separacion aprobada entre modelo general, plantilla Chome 2026, vistas y formato XLSX.
...
- [x] Hacer obligatorio resolver un alcance antes de construir cualquier XLSX.
...
- [ ] Guardar borrador automaticamente y advertir cambios no sincronizados. Parcial (incremento BLD-03): el paso "Datos basicos" autoguarda con debounce de 1,5 s reusando la misma accion de servidor del boton explicito, muestra el estado "Guardado"/"Guardando…"/"Cambios sin guardar" y advierte con `beforeunload` si hay cambios sin confirmar. Los demas pasos (objetivos, actividades, planificacion, evidencias) siguen exigiendo guardado explicito por item; cada item ya se persiste al guardarlo, pero un campo a medio escribir dentro de un formulario abierto se pierde igual que antes.
- [x] Validar progresivamente; no esperar al ultimo paso para revelar errores.
- [ ] Permitir previsualizar el programa por faena, responsable y audiencia. Parcial (incremento BLD-05): el paso de revision del constructor filtra en vivo por responsable y por audiencia (`audienceRoles`), mostrando exactamente cuantas y cuales actividades ve esa combinacion, sin crear copias. Falta la dimension de faena: hoy ninguna actividad se excluye por faena (solo su cantidad planificada puede tener una excepcion registrada via overrides), asi que una previsualizacion real por faena requeriria mostrar el efecto de esos overrides, no filtrar actividades.
...
- [ ] Modelar recurrencia/calendario sin asumir 48 semanas ni un año calendario.
...
- [ ] Guardar/recuperar borrador en cada paso. Cubierto para "Datos basicos" (`metadata-tab.test.tsx`, 3 casos); pendiente para el resto de los pasos.
...
- Base de staging con 1 programa 2026, 8 objetivos, 89 actividades, 8 vistas derivadas, 843 celdas P y total 1.035.
...
- [ ] Ampliar tipos de fuente a capacitacion, inspeccion, CPHS, EPP, emergencia, incidente, CAPA, documento/distribucion y permiso cuando aplique.
- [ ] Crear selector estructurado para enlaces manuales; no aceptar IDs pegados en texto.
...
Las 89 actividades de la plantilla 2026 tienen una decision explicita y aceptada. No puede quedar una actividad “automaticamente cubierta” sin evento, cantidad, evidencia y prueba. Las manuales deben tener razon de negocio, no ser residuos del desarrollo. Eventos, cantidades y evidencias se configuran mediante tipos reutilizables; no se codifica una condicion por numero de actividad.
...
- [x] Incluir definicion de cada indicador y permitir navegar desde el resumen a sus registros. Definicion en la hoja "Indicadores" del XLSX y en un `<details>` de la pantalla. Navegacion: cada fila enlaza al detalle del programa con la faena correcta (`#registros-pdtp`); sigue llegando a la vista general del programa, no a un recorte exacto por ese objetivo especifico (el detalle del programa no tiene filtro por objetivo todavia).
- [x] Mantener consistencia entre valores de pantalla y exportacion. Nueva pantalla `/prevencion/pdtp/[programId]/reporte` (Server Component) llama al mismo `getPdtpManagementReport` con los mismos filtros que la exportacion XLSX, y el boton "Descargar XLSX" de esa pantalla arma la URL del export con los filtros activos — mismo servicio, mismos valores, un solo lugar donde se calculan.

### 6.5 Expediente auditor

- [x] Generar ejecuciones, fuentes, evidencias, obligaciones a demanda, acciones, seguimientos, aprobaciones y cambios. Incremento BLD-09: `getPdtpAuditDossier` agrega las ocho categorias en un solo llamado (reusando `listPdtpObligations`, `listActionsByProgram`, `listFollowups`, `getPdtpApprovalProgress` ya existentes) y `GET /api/prevencion/pdtp/expediente-auditor` las exporta en ocho hojas XLSX.
...
- [ ] Navegacion KPI -> registros coincide exactamente con numerador/denominador.
- [ ] Test del “sin casos”, sobrecumplimiento, atraso y evidencia pendiente. Cubierto para on_demand/triggered: `operates triggered obligations idempotently, separates sin casos and closes only after approval` prueba `no_cases`, `with_cases`, `overdue` y el rechazo por evidencia faltante. Falta "sobrecumplimiento" para actividades `scheduled`: `compliance.ts` capa `executed` a `Math.min(executedByMonth[i], planned)` en el indicador agregado — el dato crudo en `pdtpExecutions` no se toca, pero no hay prueba ni decision explicita de que capar el AGREGADO mostrado (no solo el %) sea el comportamiento deseado frente al principio 5.2 ("conservar el real y marcar sobrecumplimiento sin caparlo silenciosamente"). No se cambio el comportamiento existente sin confirmar la intencion original.
...
- [ ] Excel/LibreOffice abre los XLSX generados sin reparaciones ni formulas inyectadas desde texto de usuario.
...
- [ ] Identificar el archivo vigente definitivo y congelar su hash.
...
- [ ] Reconciliar 8/89/8, 843, 1.035, seis E y hashes del programa.
...
- [ ] Plantilla, programa, obligacion y ejecucion conservan limites claros.
- [x] Copiar un programa no copia ejecuciones, evidencias ni firmas.
- [ ] Vistas por audiencia no duplican actividades.
- [ ] Recurrencias y disparadores producen obligaciones explicables.
- [ ] Textos largos 37/38/43/51/52 editables.
...
- capturas o XLSX de muestra cuando aplique;
...
No se debe resolver primero la apariencia del XLSX ni construir la UX alrededor de sus celdas. La secuencia prioriza seguridad y un modelo general; despues usa el archivo para validar que un caso real puede migrarse sin perdida.
...
- [ ] La importacion preserva 8/89/8, 843, 1.035 y los metadatos acordados al traducirlos al modelo general.
...
- [ ] Las 89 actividades de la plantilla 2026 tienen una matriz fuente/evento/cantidad/evidencia aprobada, implementada mediante reglas reutilizables.
...
|---|---|---|---:|---|---|---|
| 2026-07-21 | Catalogo JSON coincide con el XLSX: 8/89/8, 843 y 1.035. | Control | 0 | Por asignar | Verificado en revision | Archivo y catalogo |
| 2026-07-21 | El producto debe ser un constructor de programas; el XLSX es referencia/migracion, no interfaz objetivo. | P0 producto | 2 | Por asignar | Incorporado al plan | Aclaracion de producto |
...
| 2026-07-21 | 22 actividades sin P carecen de modo a demanda/disparado. | P1 alta | 4 | Por asignar | Abierto | Matriz semanal del XLSX |
...
| 2026-07-21 | Las salidas de gestion, auditoria y compatibilidad RE-36 no estan separadas. | P1 | 6 | Por asignar | Abierto | Builder XLSX actual |
...
- Pruebas ejecutadas: endpoint XLSX 16/16, incluida tabla explicita de siete combinaciones rol × alcance × parametro; prueba PGlite multifaena incluida en la suite ampliada a 37 casos.
...
- Pruebas ejecutadas: seguridad XLSX 4/4 para archivo real acotado, `.xls`, cuerpo corrupto, MIME invalido, expansion insegura y exceso de hojas; `npm run typecheck`, aprobado.
...
- Hallazgos nuevos: el endpoint anterior aceptaba `.xls` aunque siempre usaba el lector XLSX y entregaba el ZIP a ExcelJS sin validar expansion ni cifrado.
...
- Alcance verificado: constructor, plantillas, aprobaciones, alcance por faena, parser 2026 y borde XLSX; manual operativo actualizado en `manual/prevencion/pdtp.md` para presentar el constructor como flujo principal.
...
- Alcance implementado: cierre de dos hallazgos de Fase 2 (2.1 y parcial 2.3). Se confirmo que la referencia 2026 ya se publica como plantilla editable por el mecanismo general (sin caso especial), y se elimino el enum/condicional fijo de ocho hojas/roles 2026 en las superficies generales que lo tenian: selector de vista y resolucion de vista por defecto en la pagina de detalle del programa, y nombre de hoja/validacion de codigo en el exportador XLSX.
...
- Alcance implementado: reporte de gestion completo — servicio, exportacion XLSX y pantalla — con la estructura que comunica gestion (resumen por objetivo), no la del archivo 2026.
...
- `GET /api/prevencion/pdtp/reporte-gestion` (nuevo): mismo patron de seguridad que el export XLSX ya endurecido en Fase 1 — permiso, resolucion de faena fail-closed (una autorizada o rechazo explicito), auditoria en los cuatro desenlaces (success/denied/invalid/error), headers `Cache-Control: no-store` y `X-Content-Type-Options: nosniff`. Construye el XLSX con ExcelJS directo (dos hojas: "Resumen por objetivo" y "Indicadores") mas la hoja de metadatos de trazabilidad comun (`addExportMetadataSheet`, ya usada por otros exports del modulo prevencion), sanitizando celdas con el mismo escape de formulas (`=+-@`) que `indicadores/export/route.ts`.
- `/prevencion/pdtp/[programId]/reporte` (nuevo, pantalla): llama al mismo `getPdtpManagementReport` con los mismos filtros que la exportacion, con un boton "Descargar XLSX" que arma la URL de export con los filtros activos vigentes — un solo calculo, sin duplicar logica entre pantalla y archivo. Filtros interactivos (`ReporteGestionFilters`, client component) siguiendo el patron ya establecido de `PdtpWorksitePicker` (Select + `router.push` con querystring). Cada fila enlaza a `#registros-pdtp` en el detalle del programa.
...
- Riesgos residuales: la navegacion "Ver registros" llega a la vista general del programa con la faena correcta, no a un recorte exacto por ese objetivo (el detalle del programa no soporta filtrar por objetivo todavia). "Excel/LibreOffice abre el XLSX sin reparaciones" sigue sin verificacion automatizada para ningun export de PDTP (preexistente, no especifico de este incremento); se aplico el mismo escape de formulas que ya usan los demas exports del modulo.
...
- `GET /api/prevencion/pdtp/expediente-auditor` (nuevo): mismo patron de seguridad que `reporte-gestion` y el export original — permiso, resolucion de faena fail-closed, auditoria en los cuatro desenlaces, headers seguros. Construye el XLSX con ExcelJS directo (portada "Programa" con digest/version/estado + ocho hojas de detalle), mismo escape de formulas (`safe()`) que los demas exports del modulo prevencion.
...
1. Conservar el XLSX original en almacenamiento seguro o vincularlo a Documento SST; el lote actualmente conserva hash, metadatos y filas normalizadas, pero no el binario descargable.
2. Completar autosave de servidor por paso, indicador de cambios pendientes, recuperacion de red y pruebas de teclado/responsive; agregar preview por responsable, audiencia y faena.
3. Separar calendario general de la proyeccion anual de 48 celdas y formalizar base corporativa, faenas cubiertas, herencia y excepciones locales.
...
6. Construir la matriz 1–89 y el registro reusable de fuentes; integrar por orden Incidentes/CAPA, Capacitacion, Inspecciones/MIPER/Legal, CPHS/Documentos, EPP, Emergencias y campañas.
7. Implementar bandeja `Esta semana` unificada (ver punto 5), indicadores con denominadores explicitos, reporte de gestion XLSX y expediente auditor descargable y scoped.
