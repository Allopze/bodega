...
describe("prevention documentation canonical surface", () => {
  it("uses Documentación (/prevencion/documentacion) as the navigation surface", () => {
    ...
    expect(preventionModule.permissions).toContain("prevention:docs:approve")
    expect(preventionModule.permissions).toContain("prevention:docs:ack")
    expect(preventionModule.permissions).toContain("prevention:docs:link")
    expect(Object.keys(preventionModule.permissionMeta)).not.toContain("prevention:docs:export")
    expect(Object.keys(preventionModule.permissionMeta)).toContain("prevention:docs:approve")
    expect(Object.keys(preventionModule.permissionMeta)).toContain("prevention:docs:ack")
    expect(Object.keys(preventionModule.permissionMeta)).toContain("prevention:docs:link")
    expect(preventionModule.defaultGrants?.some((seed) => String(seed.permission) === "prevention:docs:export") ?? false).toBe(false)
    expect(preventionModule.defaultGrants?.some((seed) => String(seed.permission) === "prevention:docs:approve") ?? false).toBe(true)
    expect(preventionModule.defaultGrants?.some((seed) => String(seed.permission) === "prevention:docs:ack") ?? false).toBe(true)
    expect(preventionModule.defaultGrants?.some((seed) => String(seed.permission) === "prevention:docs:link") ?? false).toBe(true)
  })
  ...
  it("exposes the documentacion app routes and does not have legacy surfaces", () => {
    ...
    expect(actionsSource).toContain("approveSstDocumentVersionAction")
    expect(actionsSource).toContain("acknowledgeSstDocumentVersionAction")
    expect(actionsSource).toContain("createSstDocumentLinkAction")
