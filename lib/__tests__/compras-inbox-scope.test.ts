 ...
 * 2. Bajo el filtro de factura pendiente, una OC ya recibida vuelve a la
 *    bandeja: recibir sin factura es trabajo de Compras, y era el caso que la
 *    señal "Sin factura" anunciaba sin que el listado pudiera mostrarlo.
 *
...
describe("scope de la bandeja de Compras", () => {
  beforeAll(async () => {
    ...
    await order("oc-anulada",  "OC-INBOX-0003", "cancelled")
    // Eliminada: `deleteOrder` deja el mismo status con el código mutado.
    await order("oc-eliminada", "OC-INBOX-0004-DELETED-tXQzMnLA", "cancelled", now)
    await order("oc-recibida", "OC-INBOX-0005", "received")
    await order("oc-cerrada",  "OC-INBOX-0006", "closed")
  })
  ...
  it("entrega el trabajo de abastecimiento y deja la recepción terminada fuera", async () => {
    expect(await inboxCodes({ invoicePendingOnly: false })).toEqual([
      "OC-INBOX-0001", "OC-INBOX-0002", "OC-INBOX-0003",
    ])
  })
  ...
  it("conserva la anulada de verdad, que es la que la tab «Anuladas» debe contar", async () => {
    expect(await inboxCodes({ invoicePendingOnly: false })).toContain("OC-INBOX-0003")
  })

  it("bajo el filtro de factura pendiente admite la OC ya recibida, no la cerrada", async () => {
    expect(await inboxCodes({ invoicePendingOnly: true })).toEqual(["OC-INBOX-0005"])
  })
})
