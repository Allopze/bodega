...
describeIf("seed worker data", () => {
  it("loads workers from trabajadores_por_faena_actualizado.md grouped by worksite", () => {
    ...
    expect(data.workers.filter((worker) => worker.rut === "11.871.084-3")).toHaveLength(1)
    expect(data.workers.find((worker) => worker.rut === "11.871.084-3")?.worksiteName).toBe("Arauco Horcones")
  })
})
