...
function validateReviewedNormalized(value: string, sourceCode: string | null) {
  ...
  const attributes = Array.isArray(raw.attributes) ? raw.attributes.filter((attribute): attribute is { name: string; value: string } => typeof attribute?.name === "string" && typeof attribute?.value === "string") : []
  const normalized = normalizeEppRow({
    ...
    unitOfMeasure: raw.unitOfMeasure ?? "", attributes: attributes.map((attribute) => `${attribute.name}: ${attribute.value}`).join("; "),
