...
/** Misma unión de variantes que acepta `Badge`/`MetaBadge` (fuente única). */
export type BadgeTone = StateMetaInput["variant"]
 ...
 */
export interface StatusLabel extends StateMetaInput {
...
const DOCUMENT_STATUS: Record<string, StatusLabel> = {
  draft:    { label: "Pendiente de envío", variant: "neutral", hint: "Emitida en el portal pero aún no enviada al SII." },
  issued:   { label: "Emitida",            variant: "primary",    hint: "Enviada al SII; sin acuse de aceptación todavía." },
  accepted: { label: "Aceptada por el SII", variant: "success", hint: "El SII acusó recibo conforme." },
  rejected: { label: "Rechazada por el SII", variant: "danger", hint: "El SII rechazó el documento." },
  void:     { label: "Anulada",            variant: "neutral", hint: "No cuenta como facturación válida." },
  unknown:  { label: "Sin estado",         variant: "neutral", hint: "La fuente no informa estado tributario." },
}
...
const PAYMENT_STATUS: Record<string, StatusLabel> = {
  ...
  unpaid:   { label: "Pendiente de pago",  variant: "neutral", hint: "Sin pagos confirmados." },
  partial:  { label: "Pago parcial",       variant: "warning", hint: "Tiene pagos confirmados por menos del total." },
  paid:     { label: "Pagada",             variant: "success", hint: "Pagos confirmados cubren el total." },
  overpaid: { label: "Pagada de más",      variant: "signal",  hint: "Los pagos confirmados superan el total: revisar." },
}
...
const COLLECTION_STATUS: Record<string, StatusLabel> = {
  none:        { label: "Sin gestión",         variant: "neutral" },
  in_progress: { label: "En gestión",          variant: "primary" },
  committed:   { label: "Compromiso de pago",  variant: "primary", hint: "El cliente comprometió una fecha de pago." },
  disputed:    { label: "En disputa",          variant: "warning", hint: "El cliente objetó el cobro." },
  closed:      { label: "Gestión cerrada",     variant: "success" },
  written_off: { label: "Castigada",           variant: "neutral", hint: "Se dio por incobrable." },
}
...
export function dueStatusLabel(
  ...
  if (daysOverdue === null) return { label: "Sin vencimiento", variant: "neutral", hint: "Ni el documento ni el contrato definen plazo." }
  ...
  if (remaining <= 7) {
    return { label: remaining === 0 ? "Vence hoy" : `Vence en ${remaining} ${remaining === 1 ? "día" : "días"}`, variant: "warning" }
  }
  return { label: `Vence en ${remaining} días`, variant: "neutral" }
}
...
const PROPOSAL_STATUS: Record<string, StatusLabel> = {
  ...
  in_review: { label: "En revisión",          variant: "primary" },
  observed:  { label: "Observada",            variant: "warning", hint: "Devuelta con observaciones que hay que resolver." },
  approved:  { label: "Aprobada",             variant: "success" },
  ready:     { label: "Lista para facturar",  variant: "primary", hint: "Aprobada y con antecedentes completos. La emisión del DTE sigue siendo manual." },
  invoiced:  { label: "Relacionada con factura", variant: "success" },
...
const SYNC_STATUS: Record<string, StatusLabel> = {
  running: { label: "En curso",   variant: "primary" },
  success: { label: "Exitosa",    variant: "success" },
  partial: { label: "Parcial",    variant: "warning", hint: "Terminó, pero con documentos que no se pudieron procesar." },
  failed:  { label: "Con error",  variant: "danger" },
...
const VERIFICATION_STATUS: Record<string, StatusLabel> = {
  suggested: { label: "Sugerido",  variant: "neutral",    hint: "Propuesto por la plataforma. No cuenta hasta que una persona lo confirme." },
  confirmed: { label: "Confirmado", variant: "success", hint: "Validado por una persona autorizada." },
  rejected:  { label: "Descartado", variant: "neutral" },
  ...
  reverted:  { label: "Revertido", variant: "neutral", hint: "Confirmación revertida por error. El motor puede volver a proponerlo." },
}
...
const CONFIDENCE: Record<string, StatusLabel> = {
  high:   { label: "Confianza alta",  variant: "success" },
  medium: { label: "Confianza media", variant: "warning" },
  low:    { label: "Confianza baja",  variant: "neutral" },
}
...
const ACTION_OUTCOME: Record<string, StatusLabel> = {
  contacted:        { label: "Contactado",          variant: "primary" },
  no_answer:        { label: "Sin respuesta",       variant: "neutral" },
  promised_payment: { label: "Prometió pago",       variant: "primary" },
  disputed:         { label: "Objetó el cobro",     variant: "warning" },
