...
export async function getPreventionAttention(args: {
  ...
  if (args.includeEvaluations) {
    const rows = await db.select({ id: sstEvaluations.id, fecha: sstEvaluations.fechaEvaluacion, role: sstEvaluations.evaluatorRole, worksiteName: worksites.name })
  ...
  if (args.includePpa) {
    const rows = await db.select({ id: ppaSubmissions.id, workerName: ppaSubmissions.workerName, estado: ppaSubmissions.estado, createdAt: ppaSubmissions.createdAt, worksiteName: worksites.name })
