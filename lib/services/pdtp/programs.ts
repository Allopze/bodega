...
import { addPdtpChangeLogEntry, displayNameForSlug, collectResponsibleCatalog, pdtpActivityId, pdtpProgramId, pdtpScheduleId, pdtpSheetActivityId } from "./helpers"
...
export async function createPdtpProgram(input: PdtpProgramCreateInput) {
  ...
  const existingVersion = await db.select({ version: pdtpPrograms.version })
    .from(pdtpPrograms)
  ...
  const [program] = await db.insert(pdtpPrograms).values({
  ...
  if (input.copySheetsFromProgramId) {
    const sourceSheets = await db.select().from(pdtpSheets)
      .where(and(
  ...
  await addPdtpChangeLogEntry(programId, version, input.userId, "lifecycle", null, { status: "draft" }, "Programa creado.")
  return program
}
...
export async function deletePdtpProgram(programId: string, userId: string) {
  ...
  await addPdtpChangeLogEntry(programId, program.version, userId, "lifecycle", { status: "draft" }, null, "Programa eliminado.")
}
...
export async function importPdtpFromExcel(input: PdtpProgramImportInput) {
  ...
  for (const responsible of collectResponsibleCatalog(catalog)) {
    await db.insert(pdtpResponsibleCatalog).values(responsible).onConflictDoUpdate({
  ...
  const existingActivities = await db.select({ n: pdtpActivities.n }).from(pdtpActivities)
    .where(eq(pdtpActivities.programId, input.programId))
  const baseN = existingActivities.reduce((m, row) => Math.max(m, row.n), 0)
  const activityIdByNumber = new Map<number, string>()

  for (const activity of catalog.activities) {
    const newN = baseN + activityCount + 1
    const activityId = pdtpActivityId(input.programId, newN)
    activityIdByNumber.set(activity.n, activityId)
    activityCount++

    await db.insert(pdtpActivities).values({
  ...
  await addPdtpChangeLogEntry(input.programId, program.version, input.userId, "import:excel", null, { sheetCount, activityCount }, `${activityCount} actividades importadas desde Excel en ${sheetCount} hojas.`)
