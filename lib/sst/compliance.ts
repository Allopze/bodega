 ...
 *   - Para LC-SST-001 (Nuevos): Secciones 'documentacion_requisitos' (1.1) y 'induccion_capacitacion' (1.2).
 *   - Para LC-SST-002 (Seguimiento): Sección 'verificacion_documental' (3).
 * - Si hay algún bloqueo -> 'no_habilitado'.
 * - Si es LC-SST-001 (Nuevos):
 *   - >= 90% -> 'habilitado_autonomo' (CUMPLE)
 *   - < 90% -> 'no_habilitado' (NO CUMPLE)
 * - Si es LC-SST-002 (Seguimiento):
...
export function getAutomaticResultadoFinal(
  ...
  const isNuevo = code === 'LC-SST-001'

  // Blocker section check
  const isBlockerSection = (seccionId: string) => {
    if (isNuevo) {
      return seccionId === 'documentacion_requisitos' || seccionId === 'induccion_capacitacion'
    } else {
      return seccionId === 'verificacion_documental'
    }
  }
