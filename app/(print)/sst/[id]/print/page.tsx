...
export default async function SstPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canAccessWorksite(session, evaluation.worksiteId)) notFound()
    ...
    <>
      ...
      <main className="sheet" aria-label={`Acta SST — ${worker.firstName} ${worker.lastName}`}>
        ...
        <footer className="doc-footer">
          Este documento forma parte del SG-SST de Servicios Industriales Chome Ltda. ·
          Marco legal: {definition.legalFramework.join(" · ")}
        </footer>
      </main>
    </>
  )
