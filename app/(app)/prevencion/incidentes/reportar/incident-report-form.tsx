  ...
  return (
    <form action={handleSubmit} className="space-y-6">
      ...
      <section className="grid gap-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 md:grid-cols-2">
        ...
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={evacuated} onChange={(event) => setEvacuated(event.target.checked)} /> Hubo evacuación</label>
      </section>

      <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
        <label className="flex items-center gap-2 text-sm font-medium"><input type="checkbox" checked={hasPerson} onChange={(event) => setHasPerson(event.target.checked)} /> Hay una persona involucrada</label>
        {hasPerson && (
