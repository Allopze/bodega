      ...
      {["immediate_measures", "under_investigation", "pending_capa"].includes(incident.status) && can("prevention:incidents:investigate") && (
        <details className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
          ...
          <form className="mt-4 grid gap-4" action={(formData) => run(() => savePreventionIncidentInvestigationAction({
            ...
            <div className="grid gap-2 text-sm md:grid-cols-3"><Checkbox name="miperUpdateRequired" label="Requiere actualizar MIPER" /><Checkbox name="miperUpdated" label="MIPER actualizado" /><span /><Checkbox name="procedureUpdateRequired" label="Requiere procedimiento" /><Checkbox name="procedureUpdated" label="Procedimiento actualizado" /><span /><Checkbox name="trainingRequired" label="Requiere capacitación" /><Checkbox name="trainingCompleted" label="Capacitación completada" /></div>
            <div className="space-y-2"><Label>Motivo de actualización</Label><Input name="reason" required minLength={5} /></div>
            <Checkbox name="complete" label={<span className="font-medium">Declarar investigación completa</span>} />
