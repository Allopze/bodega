...
export default async function LegalRequirementDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PageContainer width="workbench"><PageHeader title={`${detail.requirement.code} · ${detail.requirement.article}`} description="Fuente y evaluación de aplicabilidad del requisito." breadcrumb={<Breadcrumbs items={[{ label: "Requisitos legales", href: "/prevencion/requisitos-legales" }, { label: detail.requirement.code }]} />} />
    <div className="space-y-4"><section className="rounded-lg border p-5"><div className="flex gap-2"><Badge variant={legalStatusVariant(detail.inForce ? detail.requirement.status : "superseded")}>{detail.inForce ? "Vigente" : detail.requirement.status === "published" ? "Vigencia terminada" : LEGAL_REQUIREMENT_STATUS_LABELS[detail.requirement.status] ?? detail.requirement.status}</Badge><Badge>{SOURCE_TYPE[detail.requirement.sourceType] ?? detail.requirement.sourceType}</Badge><Badge variant="outline">Vigencia {formatDate(detail.requirement.validFrom)}{detail.requirement.validTo ? ` a ${formatDate(detail.requirement.validTo)}` : " en adelante"}</Badge></div><h2 className="mt-3 text-lg font-semibold">{detail.requirement.requirement}</h2><p className="mt-2 text-sm">{detail.requirement.authority} · {detail.requirement.sourceTitle} · {detail.requirement.sourceReference}</p><p className="mt-1 text-sm text-[var(--color-text-subtle)]">Rol de Chome: {detail.requirement.chomeRole}</p></section><section className="rounded-lg border p-5"><h2 className="font-semibold">Aplicabilidad en tu alcance</h2><div className="mt-3 space-y-2">{detail.applicabilities.length === 0 ? <p className="text-sm text-[var(--color-text-subtle)]">Sin decisiones visibles.</p> : detail.applicabilities.map(({ applicability, worksiteName }) => <div key={applicability.id} className="rounded border p-3 text-sm"><div className="flex gap-2"><strong>{worksiteName}</strong><Badge variant={legalStatusVariant(applicability.applicabilityStatus)}>{LEGAL_APPLICABILITY_STATUS_LABELS[applicability.applicabilityStatus] ?? applicability.applicabilityStatus}</Badge><Badge variant={legalStatusVariant(applicability.complianceStatus)}>{LEGAL_COMPLIANCE_STATUS_LABELS[applicability.complianceStatus] ?? applicability.complianceStatus}</Badge></div><p className="mt-1">{applicability.rationale}</p><p className="mt-1 text-[var(--color-text-subtle)]">Evidencia: {applicability.evidenceReference ?? "pendiente"}</p></div>)}</div></section></div>
  </PageContainer>
}
