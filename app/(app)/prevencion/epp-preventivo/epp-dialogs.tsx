    ...
    <Dialog open={open} onOpenChange={(value) => {
      ...
      <DialogContent>
        <form onSubmit={submit} className="space-y-4">
          ...
          {scopeType === "position" && (
            <Field label="Cargo" hint="Debe coincidir con el cargo registrado del trabajador."><input name="scopeValue" required aria-label="Cargo" className="h-10 rounded-md border border-[var(--color-border-control)] bg-transparent px-3 text-sm" /></Field>
          )}
