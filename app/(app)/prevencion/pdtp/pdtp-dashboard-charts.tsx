    ...
    <div className="space-y-6">
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <ChartSection title="Cumplimiento por faena" description="Comparativa de todas las faenas autorizadas. El selector de faena filtra la tendencia y el avance por eje.">
          {!hasWorksiteExecutions ? <NoAccreditedExecutions href={operationalHref} /> : worksiteCompliance.length === 0 ? <EmptyState compact title="Sin faenas con planificación" description="Cuando existan actividades planificadas aparecerán aquí." /> : <ChartContainer config={worksiteConfig} className="h-64 w-full"><BarChart data={worksiteCompliance} layout="vertical" margin={{ top: 10, right: 10, left: 20, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" domain={[0, 100]} tickFormatter={(value) => `${value}%`} tickLine={false} axisLine={false} /><YAxis dataKey="name" type="category" width={110} tickLine={false} axisLine={false} tick={{ fontSize: 11 }} /><ChartTooltip content={<ChartTooltipContent formatter={(value, _name, item) => [item?.payload ? `${value}% (${item.payload.executed}/${item.payload.scheduled})` : `${value}%`, "% Cumplimiento"]} />} /><Bar dataKey="percent" radius={[0, 4, 4, 0]} barSize={18}>{worksiteCompliance.map((entry) => <Cell key={entry.name} fill={entry.percent >= 80 ? "var(--color-success)" : entry.percent >= 50 ? "var(--color-warning-ink)" : "var(--color-danger)"} />)}</Bar></BarChart></ChartContainer>}
        </ChartSection>
      </div>
