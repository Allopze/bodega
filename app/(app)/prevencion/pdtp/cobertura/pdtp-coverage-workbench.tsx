...
function LinkSourceDialog({ activityId, worksites, riskControls, legalRequirements }: { activityId: string; worksites: Array<{ id: string; name: string }>; riskControls: Source[]; legalRequirements: Source[] }) {
  const [open, setOpen] = useState(false); const [worksiteId, setWorksiteId] = useState(worksites[0]?.id ?? ""); const [sourceType, setSourceType] = useState("risk_control"); const operation = useOperation()
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button size="sm">Vincular fuente</Button></DialogTrigger><DialogContent><form onSubmit={submit} className="space-y-4"><DialogHeader><DialogTitle>Vincular origen de la medida</DialogTitle><DialogDescription>El vínculo no cierra el requisito ni reduce el riesgo; sólo demuestra cobertura programática.</DialogDescription></DialogHeader><Field label="Faena"><select value={worksiteId} onChange={(event) => setWorksiteId(event.target.value)} className="h-10 rounded-md border bg-transparent px-3">{worksites.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><Field label="Tipo"><select value={sourceType} onChange={(event) => setSourceType(event.target.value)} className="h-10 rounded-md border bg-transparent px-3"><option value="risk_control">Control MIPER</option><option value="legal_requirement">Requisito legal</option><option value="incident_capa">Incidente / CAPA</option><option value="audit">Auditoría</option><option value="contractual_obligation">Obligación contractual</option><option value="internal_objective">Objetivo interno</option></select></Field>{options.length > 0 ? <Field label="Fuente"><select name="sourceId" className="h-10 rounded-md border bg-transparent px-3">{options.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></Field> : <Field label="Identificador de fuente"><Input name="sourceId" required={!["audit", "contractual_obligation", "internal_objective"].includes(sourceType)} placeholder="ID CAPA, auditoría, contrato u objetivo" /></Field>}<Field label="Justificación"><Textarea name="justification" required minLength={10} /></Field>{operation.message && <p role="status" className="text-sm">{operation.message}</p>}<DialogFooter><Button type="submit" disabled={operation.pending}>Crear vínculo</Button></DialogFooter></form></DialogContent></Dialog>
}
