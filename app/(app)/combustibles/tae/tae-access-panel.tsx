...
import { createTaeLoadingPointAction, createTaePublicLinkAction, revokeTaePublicLinkAction } from "./actions"
import { toast } from "@/lib/toast"

interface Worksite { id: string; name: string }
interface LoadingPoint { id: string; worksiteId: string; name: string; type: string }
interface ExistingLink { id: string; label: string; worksiteName: string; loadingPointName: string | null; revokedAt: string | null }

export function TaeAccessPanel({ worksites, loadingPoints, existingLinks }: { worksites: Worksite[]; loadingPoints: LoadingPoint[]; existingLinks: ExistingLink[] }) {
  ...
  function createPoint() {
    startTransition(async () => {
      const result = await createTaeLoadingPointAction({ worksiteId, name: newPointName, type: "tae" })
  ...
  return (
    <section className="border border-(--color-border) bg-(--color-surface) p-4">
      ...
      <div className="mt-3 flex max-w-md items-end gap-2"><div className="min-w-0 flex-1"><Label htmlFor="tae-new-point">Nuevo punto de carga</Label><Input id="tae-new-point" value={newPointName} onChange={(event) => setNewPointName(event.target.value)} placeholder="Ej. Surtidor principal" /></div><Button type="button" variant="secondary" size="sm" onClick={createPoint} disabled={!worksiteId || !newPointName.trim() || pending}><Plus size={14} /> Crear</Button></div>
