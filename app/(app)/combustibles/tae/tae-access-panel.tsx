  ...
  return (
    <section className="border border-(--color-border) bg-(--color-surface) p-4">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        <div><Label>Faena</Label><Select value={worksiteId} onValueChange={(value) => { setWorksiteId(value); setLoadingPointId("") }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{worksites.map((worksite) => <SelectItem key={worksite.id} value={worksite.id}>{worksite.name}</SelectItem>)}</SelectContent></Select></div>
        <div><Label>Punto de carga</Label><Select value={loadingPointId} onValueChange={setLoadingPointId}><SelectTrigger><SelectValue placeholder="Selecciona un punto" /></SelectTrigger><SelectContent>{points.map((point) => <SelectItem key={point.id} value={point.id}>{point.name}</SelectItem>)}</SelectContent></Select></div>
      ...
      <div className="mt-3 flex max-w-2xl items-end gap-2"><div className="min-w-0 flex-1"><Label htmlFor="tae-new-point">Nuevo punto de carga</Label><Input id="tae-new-point" value={newPointName} onChange={(event) => setNewPointName(event.target.value)} placeholder="Ej. Surtidor principal" /></div><div className="min-w-0 flex-1"><Label>Vasija (opcional)</Label><Select value={newPointStorageId} onValueChange={setNewPointStorageId}><SelectTrigger><SelectValue placeholder="Sin vincular" /></SelectTrigger><SelectContent>{worksiteStorageLocations.map((location) => <SelectItem key={location.id} value={location.id}>{location.name}</SelectItem>)}</SelectContent></Select></div><Button type="button" variant="secondary" size="sm" onClick={createPoint} disabled={!worksiteId || !newPointName.trim() || pending}><Plus size={14} /> Crear</Button></div>
      {points.length > 0 && <div className="mt-4 border-t border-(--color-border) pt-4"><p className="text-eyebrow">Vasija de cada punto de carga</p><p className="mt-1 text-xs text-(--color-text-muted)">Sin este enlace, lo que la PWA entrega en un punto no se descuenta del saldo de ninguna vasija.</p><div className="mt-2 space-y-2">{points.map((point) => <div key={point.id} className="flex items-center justify-between gap-3 text-sm"><span>{point.name}</span><Select value={point.storageLocationId ?? "none"} onValueChange={(value) => setPointStorage(point.id, value)}><SelectTrigger className="w-56"><SelectValue placeholder="Sin vincular" /></SelectTrigger><SelectContent><SelectItem value="none">Sin vincular</SelectItem>{worksiteStorageLocations.map((location) => <SelectItem key={location.id} value={location.id}>{location.name}</SelectItem>)}</SelectContent></Select></div>)}</div></div>}
