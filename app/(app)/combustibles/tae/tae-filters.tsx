  ...
  return (
    <form method="get" className="mb-5 border border-(--color-border) bg-(--color-surface-2) p-4">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        ...
        <div><Label htmlFor="tae-worksite">Faena</Label><select id="tae-worksite" name="faena" defaultValue={values.worksiteId} className={selectClass}><option value="">Todas</option>{worksites.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
        <div><Label htmlFor="tae-point">Punto</Label><select id="tae-point" name="punto" defaultValue={values.loadingPointId} className={selectClass}><option value="">Todos</option>{loadingPoints.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
        <div><Label htmlFor="tae-status">Estado</Label><select id="tae-status" name="estado" defaultValue={values.status} className={selectClass}><option value="">Todos</option><option value="submitted">Recibida</option><option value="observed">Observada</option><option value="validated">Validada</option><option value="voided">Anulada</option></select></div>
      </div>
