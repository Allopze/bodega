  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap gap-2 pb-3">
        {[1, 2, 3].map((i) => (
          <Skeleton  key={i} className="h-8 w-44  rounded bg-(--color-surface-2)" />
        ))}
      </div>
      ...
      <div className="mb-4 grid gap-3 pb-4 md:grid-cols-5">
        {[1, 2, 3, 4, 5].map((i) => (
          <Skeleton  key={i} className="h-9  rounded bg-(--color-surface-2)" />
        ))}
      </div>
      ...
      <div className="mb-6 grid gap-5 lg:grid-cols-2">
        {[1, 2].map((i) => (
          <Skeleton  key={i} className="h-64  rounded border border-(--color-border) bg-(--color-surface)" />
        ))}
      </div>
      ...
      <div className="space-y-2">
        {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
          <Skeleton  key={i} className="h-10  rounded bg-(--color-surface-2)" />
        ))}
      </div>
    </>
  )
