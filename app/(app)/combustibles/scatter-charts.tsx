...
export function LitersVsKmChart({ points }: { points: ScatterPoint[] }) {
  ...
  return (
    <div className="h-80" aria-label="Dispersión litros vs kilometraje">
      <ResponsiveContainer width="100%" height="100%" debounce={200}>
        <ReScatter margin={{ top: 8, right: 12, left: 0, bottom: 36 }}>
          <CartesianGrid stroke={GRID_COLOR} strokeDasharray="2 5" />
          <XAxis dataKey="x" name="Km" unit=" km" type="number" {...chartAxisProps()} label={{ value: "Odómetro (km)", position: "bottom", offset: -4, fill: "var(--color-text-muted)", fontSize: 11 }} />
            ...
            content={({ active, payload }) => {
              ...
              return (
                <div className="min-w-36 border border-[var(--color-border-strong)] bg-[var(--color-surface)] px-3 py-2 shadow-[var(--shadow-md)] text-xs">
                  ...
                  <div className="mt-1 flex justify-between gap-4"><span className="text-[var(--color-text-muted)]">Odómetro</span><span className="font-mono">{point.x.toLocaleString("es-CL")} km</span></div>
...
export function LitersVsHourMeterChart({ points }: { points: ScatterPoint[] }) {
  ...
  return (
    <div className="h-80" aria-label="Dispersión litros vs horómetro">
      <ResponsiveContainer width="100%" height="100%" debounce={200}>
        <ReScatter margin={{ top: 8, right: 12, left: 0, bottom: 36 }}>
          <CartesianGrid stroke={GRID_COLOR} strokeDasharray="2 5" />
          <XAxis dataKey="x" name="Horas" unit=" h" type="number" {...chartAxisProps()} label={{ value: "Horómetro (h)", position: "bottom", offset: -4, fill: "var(--color-text-muted)", fontSize: 11 }} />
          <YAxis dataKey="y" name="Litros" unit=" L" width={48} type="number" {...chartAxisProps()} label={{ value: "Litros", angle: -90, position: "insideLeft", offset: -4, fill: "var(--color-text-muted)", fontSize: 11 }} />
          <ZAxis range={[52, 52]} />
          <Tooltip
            content={({ active, payload }) => {
              ...
              return (
                <div className="min-w-36 border border-[var(--color-border-strong)] bg-[var(--color-surface)] px-3 py-2 shadow-[var(--shadow-md)] text-xs">
                  ...
                  {point.type && <p className="text-[var(--color-text-muted)]">{point.type}</p>}
                  {point.worksite && <p className="text-[var(--color-text-muted)]">{point.worksite}</p>}
                  <div className="mt-1 flex justify-between gap-4"><span className="text-[var(--color-text-muted)]">Horómetro</span><span className="font-mono">{point.x.toLocaleString("es-CL")} h</span></div>
                  <div className="flex justify-between gap-4"><span className="text-[var(--color-text-muted)]">Litros</span><span className="font-mono">{point.y.toLocaleString("es-CL")} L</span></div>
                </div>
              )
            }}
