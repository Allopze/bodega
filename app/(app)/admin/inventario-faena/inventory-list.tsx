...
function AssignResourceDialog({ point, resources, today }: { point: CoveragePointRow; resources: InventoryRow[]; today: string }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); operation.run(() => assignEmergencyResourceAction({ pointId: point.id, resourceId, reason: String(form.get("reason") ?? "") }), () => { setOpen(false); setResourceId("") }) }}>
