  ...
  return (
    <section aria-labelledby="cola-operacional" className="space-y-4">
      <div className="flex flex-col gap-2 border-b border-[var(--color-border)] pb-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <time dateTime={result.refreshedAt} className="text-xs text-[var(--color-text-subtle)]">Actualizada {formatDateTime(result.refreshedAt)}</time>
      </div>
      ...
      <form onSubmit={submitSearch} className="flex flex-col gap-2 sm:flex-row" role="search">
        ...
        <input
          ...
          className="h-11 min-w-0 flex-1 rounded-[var(--radius-lg)] border border-[var(--color-border-control)] bg-[var(--color-surface)] px-3 text-sm text-[var(--color-text)] outline-none placeholder:text-[var(--color-text-subtle)] focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary-line)] sm:h-9"
        />
        <Button type="submit" variant="secondary"><FunnelSimple size={15} />Buscar</Button>
      </form>
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Select value={searchParams.get("sort") ?? "priority"} onValueChange={(value) => update({ sort: value })}>
          <SelectTrigger aria-label="Ordenar cola"><SelectValue placeholder="Orden" /></SelectTrigger>
      ...
      <details className="rounded-[var(--radius)] border border-[var(--color-border)] px-3 py-2">
        <summary className="cursor-pointer text-sm font-medium text-[var(--color-text)]">Más filtros{activeFilters > 0 ? ` (${activeFilters})` : ""}</summary>
      ...
      {result.items.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-[var(--radius-lg)] border border-[var(--color-border)]">
          <table className="w-full min-w-[980px] border-collapse text-left text-[13px]" aria-label="Pendientes operacionales">
            <thead className="bg-[var(--color-surface-2)] text-[var(--color-text-muted)]"><tr><th className="px-4 py-3 th-type">Prioridad</th><th className="px-4 py-3 th-type">Tarea</th><th className="px-4 py-3 th-type">Módulo</th><th className="px-4 py-3 th-type">Faena</th><th className="px-4 py-3 th-type">Responsable</th><th className="px-4 py-3 th-type">Estado</th><th className="px-4 py-3 th-type">Antigüedad</th><th className="px-4 py-3 th-type">Vencimiento</th><th className="px-4 py-3"><span className="sr-only">Acciones</span></th></tr></thead>
            <tbody>{result.items.map((item) => <tr key={item.id} className="border-t border-[var(--color-border)] align-middle hover:bg-[var(--color-surface-2)]"><td className="px-4 py-3"><PriorityBadge priority={item.priority} /></td><td className="px-4 py-3"><Link href={item.href} className="font-medium text-[var(--color-text)] hover:text-[var(--color-primary-ink)] hover:underline">{item.title}</Link><p className="mt-0.5 text-xs text-[var(--color-text-muted)]">{item.code && <span>{item.code} · </span>}{item.subtitle}</p></td><td className="px-4 py-3 text-[var(--color-text-muted)]">{MODULE_LABELS[item.module]}</td><td className="px-4 py-3 text-[var(--color-text-muted)]">{item.worksiteName}</td><td className="px-4 py-3 text-[var(--color-text-muted)]">{item.assignee ? <span>{item.assignee.name}{item.assignee.source === "assignment" && <span className="block text-xs text-[var(--color-text-subtle)]">Asignación</span>}</span> : "Sin responsable"}</td><td className="px-4 py-3"><Badge variant={item.blocked ? "danger" : item.status === "overdue" ? "warning" : "info"} size="sm">{item.blocked ? "Bloqueada · " : ""}{item.statusLabel}</Badge></td><td className="px-4 py-3 text-[var(--color-text-muted)]">{relativeAge(item.createdAt)}</td><td className="px-4 py-3 text-[var(--color-text-muted)]">{item.effectiveDueAt ? <><time dateTime={item.effectiveDueAt}>{formatDate(item.effectiveDueAt)}</time><span className="block text-xs text-[var(--color-text-subtle)]">{item.dueSource === "commitment" ? "Compromiso" : "Fecha origen"}</span></> : "Sin fecha"}</td><td className="px-4 py-3"><div className="flex items-center justify-end gap-1"><Button asChild size="sm" variant="ghost"><Link href={item.href}>{item.ctaLabel}<ArrowRight size={14} /></Link></Button>{canAssign && item.assignable && <WorkAssignmentControl item={item} />}</div></td></tr>)}</tbody>
          </table>
        </div>
      )}
