...
const COLUMNS = [
  ...
  { key: "itemCount",    label: "Servicios", sortable: true, numeric: true, width: "w-24" },
  ...
  return (
    <>
      <DataTable
        ...
        emptyTitle="Sin solicitudes de servicios"
        emptyDescription="Las solicitudes de servicios externos aparecerán aquí."
        emptyAction={
          canCreate ? (
            hasWorksites ? (
              <Button variant="primary" size="sm" asChild>
                <Link href="/servicios/nueva">
        ...
        renderMobileCard={(row) => {
          ...
          return (
            <Link href={`/servicios/${r.id}`} className="block">
              <article className="rounded-[var(--radius-2xl)] bg-[var(--color-surface)] shadow-[var(--shadow-card)] p-4 transition-colors duration-[var(--duration-fast)] hover:bg-[var(--color-primary-tint)]">
                ...
                <dl className="mt-3 grid grid-cols-3 gap-x-3 gap-y-2 text-xs">
                  ...
                  <div>
                    <dt className="text-[var(--color-text-subtle)]">Servicios</dt>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader className="flex flex-col items-center text-center">
            ...
            <DialogDescription className="mt-2 text-sm text-center">
              No tienes faenas activas asignadas a tu cuenta. Contacta a un administrador para que te asigne una faena antes de poder crear una solicitud de servicios.
            </DialogDescription>
          </DialogHeader>
