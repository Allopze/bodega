...
export async function saveDraftAction(
  ...
  catch { return { ok: false, message: "Sin permisos para crear solicitudes de servicios" } }
  ...
  try {
    const requestId = await persistServiceDraft(session, {
      id:            d.id,
    ...
    logger.error("[servicios/saveDraftAction]", e)
...
export async function submitRequestAction(
  ...
  let session
  try { session = await requirePermission("servicios:submit") }
  catch { return { ok: false, message: "Sin permisos para enviar solicitudes de servicios" } }
  ...
  try {
    ...
    void getUserIdsWithPermission("servicios:approve").then((approverIds) =>
      notifyManyUser(approverIds, {
        type:       "request_submitted",
        title:      "Nueva solicitud de servicios pendiente",
        body:       "Hay una solicitud de servicios externos con cotizaciones esperando selección.",
        entityType: "purchase_request",
        entityId:   requestId,
        entityHref: `/servicios/${requestId}`,
      })
    )

    revalidatePath(REVALIDATE)
    redirect(`/servicios/${requestId}`)
  } catch (e) {
    if (e instanceof Error && e.message.includes("NEXT_REDIRECT")) throw e
    logger.error("[servicios/submitRequestAction]", e)
...
export async function uploadQuotationAction(
  ...
  const parsed = serviceQuotationUploadSchema.safeParse({
  ...
  try {
    ...
    revalidatePath(`/servicios/${d.requestId}`)
    return { ok: true, message: "Cotización agregada" }
  } catch (e) {
    logger.error("[servicios/uploadQuotationAction]", e)
...
export async function deleteQuotationAction(
  ...
  try {
    await deleteServiceQuotation({
      quotationId,
      userId:    session.user.id,
      userEmail: session.user.email ?? undefined,
    })
    revalidatePath(`/servicios/${requestId}`)
    return { ok: true, message: "Cotización eliminada" }
  } catch (e) {
    logger.error("[servicios/deleteQuotationAction]", e)
...
export async function selectQuotationAction(
  ...
  let session
  try { session = await requirePermission("servicios:approve") }
  catch { return { ok: false, message: "Sin permisos para aprobar cotizaciones" } }

  const parsed = selectServiceQuotationSchema.safeParse({
    requestId:   formData.get("requestId"),
  ...
  try {
    ...
    void notifySafe({
      ...
      entityHref: `/servicios/${d.requestId}`,
    })
    ...
    logger.error("[servicios/selectQuotationAction]", e)
...
export async function cancelRequestAction(
  ...
  const parsed = cancelServiceSchema.safeParse({
    requestId: formData.get("requestId"),
  ...
  try {
    await cancelServiceRequest(d.requestId, session.user.id, d.reason, {
      userEmail: session.user.email ?? undefined,
    })
    revalidatePath(REVALIDATE)
    redirect(REVALIDATE)
    ...
    if (e instanceof Error && e.message.includes("NEXT_REDIRECT")) throw e
    logger.error("[servicios/cancelRequestAction]", e)
