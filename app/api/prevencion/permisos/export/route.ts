...
import { buildPermitsExport } from "@/lib/services/prevention-permits"
...
export async function GET() {
  ...
  if (!can(session, "prevention:permits:view")) return NextResponse.json({ error: "Sin permisos" }, { status: 403 })

  const scope = resolveWorksiteScope(session)
  const worksiteIds: string[] | "all" = scope.mode === "all" ? "all" : scope.mode === "some" ? scope.ids : []

  try {
    const report = await buildPermitsExport(worksiteIds)
    ...
    logger.error("[prevencion/permisos/export]", err)
