/**
 * GET /api/prevencion/incidentes/export
 * Exporta los incidentes y acciones correctivas del scope como XLSX.
 */
...
import { buildIncidentExport } from "@/lib/services/prevention-incidents"
...
export async function GET() {
  ...
  if (!can(session, "prevention:incidents:view")) {
  ...
  try {
    const report = await buildIncidentExport(worksiteIds)
    ...
    logger.error("[prevencion/incidentes/export]", err)
