...
/**
 * POST /api/prevencion/campanas/evidence
 *
 * Sube la evidencia de difusión de una campaña (foto, lista de asistencia,
 * acta). Devuelve `{ path, checksumSha256 }`; la ruta es lo que el formulario
 * manda como `evidenceUrl` al marcar la campaña como hecha.
 *
 * El permiso es el del acto que la usa —marcar la campaña—, no el de verla:
 * quien sólo consulta no sube archivos.
 */
export async function POST(request: Request) {
  const guard = await guardPermission("prevention:campaign:manage")
  ...
  try {
    ...
    logger.error("[prevencion/campanas/evidence]", error)
