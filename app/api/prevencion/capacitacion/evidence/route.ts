...
import {
  TRAINING_OCCURRENCE_MAX_FILE_SIZE,
...
export async function POST(request: Request) {
  ...
  if (Number.isFinite(contentLength) && contentLength > TRAINING_OCCURRENCE_MAX_REQUEST_SIZE) {
    return NextResponse.json({
      error: `La petición supera el máximo permitido de ${Math.round(TRAINING_OCCURRENCE_MAX_FILE_SIZE / 1024 / 1024)} MB por archivo.`,
    }, { status: 413 })
  }
  ...
  const occurrenceId = String(form.get("occurrenceId") ?? "").trim()
  const files = form.getAll("file")
  const file = files.length === 1 ? files[0] : null
  if (!occurrenceId) return NextResponse.json({ error: "Falta la ocurrencia de capacitación." }, { status: 400 })
  if (files.length !== 1) return NextResponse.json({ error: "Adjunta exactamente un archivo por solicitud." }, { status: 400 })
  if (!(file instanceof File)) return NextResponse.json({ error: "Falta el archivo." }, { status: 400 })
  if (file.size > TRAINING_OCCURRENCE_MAX_FILE_SIZE) {
    return NextResponse.json({
      error: `El archivo supera el máximo permitido de ${Math.round(TRAINING_OCCURRENCE_MAX_FILE_SIZE / 1024 / 1024)} MB.`,
    }, { status: 400 })
  }

  try {
    const result = await uploadTrainingOccurrenceEvidence({
      occurrenceId,
      fileName: file.name,
      fileSize: file.size,
    ...
    logger.error("[prevencion/capacitacion/evidence]", error)
    return NextResponse.json({
