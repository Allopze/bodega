...
import { buildAlcoholTestsExport } from "@/lib/services/prevention-alcohol-tests"
...
export async function GET() {
  ...
  if (!can(session, "prevention:alcohol_tests:view")) return NextResponse.json({ error: "Sin permisos" }, { status: 403 })

  const scope = resolveWorksiteScope(session)
  const worksiteIds: string[] | "all" = scope.mode === "all" ? "all" : scope.mode === "some" ? scope.ids : []

  try {
    const report = await buildAlcoholTestsExport(worksiteIds)
    ...
    logger.error("[prevencion/alcotest/export]", err)
