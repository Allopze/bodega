...
describe("GET PDTP Excel", () => {
  ...
  it("derives the only authorized worksite and exports with defense-in-depth scope", async () => {
    mockAuth.mockResolvedValue(session())

    const response = await GET(request("?year=2026&hoja=cphs&programId=p1"))
    ...
    expect(mockAudit).toHaveBeenCalledWith(expect.objectContaining({
      ...
      entityId: "p1",
      newState: expect.objectContaining({ result: "success", requestedWorksiteId: "w1" }),
    }))
  })
  ...
  it("lets global users export one explicitly selected worksite", async () => {
    mockAuth.mockResolvedValue(session({ isGlobal: true, worksiteIds: [] }))
    const response = await GET(request("?year=2026&faena=w9"))
    expect(response.status).toBe(200)
