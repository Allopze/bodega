...
import { buildEquipmentReportsExport } from "@/lib/services/prevention-equipment"
...
export async function GET() {
  ...
  if (!can(session, "prevention:equipment_reports:view")) return NextResponse.json({ error: "Sin permisos" }, { status: 403 })

  const scope = resolveWorksiteScope(session)
  const worksiteIds: string[] | "all" = scope.mode === "all" ? "all" : scope.mode === "some" ? scope.ids : []

  try {
    const report = await buildEquipmentReportsExport(worksiteIds)
    ...
    logger.error("[prevencion/equipos/reportes/export]", err)
