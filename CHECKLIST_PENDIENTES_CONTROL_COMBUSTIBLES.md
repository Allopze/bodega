...
Fecha de consolidación: 2026-07-13. Última actualización: 2026-07-13 (selección cruzada real en `/combustibles`: barras de patente/proveedor filtran indicadores+gráficos+tabla en la misma página, con chips de origen retirables).
...
- Validación de esta pasada (secciones 4, 5, 6 y 7): sólo `npx tsc --noEmit` sobre el árbol completo — sin errores, tras corregir en el proceso un `LabelList.formatter` con tipo incompatible, un `ErrorBar` de Recharts mal alimentado (esperaba un valor simétrico único, no una tupla de límites), y un bug de correctitud detectado antes de enviarse: el diseño original cableaba selección cruzada por faena usando texto libre importado como si fuera un `worksiteId`, lo que habría producido un filtro roto en silencio — se descartó esa parte en vez de forzarla. **No se corrió ESLint ni la suite de pruebas** — a pedido explícito, para ejecutar todo junto al finalizar los 320 pendientes restantes. No se verificó en navegador (Chrome no instalado en este entorno) — ni los gráficos ni la selección cruzada tuvieron inspección visual real, sólo verificación de tipos y de la API de Recharts contra su código fuente instalado.
...
- [ ] Implementar selección cruzada desde filas. No implementado — ni la bitácora ni la tabla de detalle de consumo permiten "clic en fila → aplica como filtro".
