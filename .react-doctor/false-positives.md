...
- `react-doctor/nextjs-no-a-element` in `app/(app)/admin/productos/product-actions.tsx`, `app/(app)/admin/proveedores/supplier-actions.tsx`, and `app/(app)/admin/trabajadores/worker-actions.tsx`: the anchors target authenticated XLSX export route handlers. They intentionally trigger browser downloads and are not page navigation links.
