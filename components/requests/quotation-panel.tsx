...
export interface QuotationRow {
  ...
  supplierName?:    string | null   // from joined supplier catalog
  fileName:         string
  totalAmount:      number
...
export interface QuotationPanelProps {
  ...
  canUpload:     boolean
  canApprove:    boolean
  /** Base path for file download links, e.g. "/api/repuestos/quotaciones" */
  downloadBase:  string
  uploadAction:  QuotationActionFn
  deleteAction:  QuotationActionFn
  selectAction?: QuotationActionFn
}
...
export function QuotationPanel({
  ...
  const [uploadState, uploadDispatch, uploadPending] = useActionState(
    async (prev: { ok: boolean; message?: string }, formData: FormData): Promise<{ ok: boolean; message?: string }> => {
      const res = await uploadAction(prev, formData)
      if (res.ok) {
  ...
  const [_deleteState, deleteDispatch, deletePending] = useActionState(
    async (prev: { ok: boolean; message?: string }, formData: FormData): Promise<{ ok: boolean; message?: string }> => {
      const res = await deleteAction(prev, formData)
      if (res.ok) {
  ...
  const [_selectState, selectDispatch, selectPending] = useActionState(
    async (prev: { ok: boolean; message?: string }, formData: FormData): Promise<{ ok: boolean; message?: string }> => {
      if (!selectAction) return prev
      const res = await selectAction(prev, formData)
      if (res.ok) {
    ...
    <div className="rounded-[var(--radius-2xl)] bg-[var(--color-surface)] shadow-[var(--shadow-card)] p-6 space-y-4">
          ...
          {quotations.map((q) => (
            ...
            >
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                {isPendingReview && canApprove && q.status === "pending" && selectAction && (
                  <Button
                    type="button"
                    variant="secondary"
                    size="sm"
      ...
      {isApproved && (
        <p className="text-xs text-[var(--color-success-ink)] font-medium">
          ✓ Cotización aprobada. Ítems listos para orden de compra.
        </p>
      )}
      ...
      <Dialog open={showUpload} onOpenChange={setShowUpload}>
        <DialogContent className="sm:max-w-lg">
          ...
          <form action={uploadDispatch} className="space-y-4">
            <input type="hidden" name="requestId" value={requestId} />
            {requestType && <input type="hidden" name="requestType" value={requestType} />}
            ...
            <Field label="Observaciones">
              <Input
                name="notes"
                placeholder="Condiciones de pago, plazo de entrega..."
                maxLength={300}
              />
            </Field>
