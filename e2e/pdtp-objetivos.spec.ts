...
test.describe("PDTP — objetivos y filtro de actividades", () => {
  test("crea un objetivo, lo asigna a una actividad y el visor de actividades filtra por ese objetivo", async ({ page }) => {
    ...
    // Asigna el objetivo recién creado a "Actividad ajustable anual E2E"
    ...
    await activityRow.getByRole("combobox", { name: "Objetivo de la actividad 90" }).click()
    ...
    await expect(activityRow.getByRole("combobox", { name: "Objetivo de la actividad 90" })).toContainText("Objetivo E2E")
    ...
    await expect(page.getByRole("row").filter({ hasText: "Actividad ajustable anual E2E" })).toBeVisible()
    await expect(page.getByRole("row").filter({ hasText: "Actividad sin objetivo E2E" })).not.toBeVisible()
  })
})
