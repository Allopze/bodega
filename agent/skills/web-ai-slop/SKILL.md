...
Run this. Don't just keep the list open as reference while defaulting anyway — that's how the original version of this skill still produced slop even when "consulted."

### 1a. Anchor before drafting

Name one concrete thing this output has to be true to: the actual subject or product, its audience, and the single job this specific page, component, doc, or config has to do. If the brief already pins a direction down, follow it exactly — don't substitute your own references over explicit instructions.

Where the brief leaves room, name **1–2 specific, real references** to triangulate against — not a vibe ("modern," "clean," "trustworthy," "professional") but an actual named thing: a product's information density, a print tradition's column rules, a specific competitor you're deliberately differentiating from, a material or instrument from the subject's own world. One reference invites a clone. A named blend of two forces something new: "this hardware datasheet's directness, crossed with a farmers-almanac's typographic warmth" gives you two anchors to interpolate between, not one to copy.

This is the step the original checklist-only version lacked. A pure filter tells a model what not to produce; it never tells it what to produce instead, so the "instead" often quietly reverts to the same center by a different route. Naming the reference is what actually moves the output.

### 1b. Draft

Produce the real thing — copy, layout, component, config, doc — working from the anchor, not from the first pattern that comes to mind.

### 1c. Filter

Check the draft against the relevant reference file(s) below. **Two or more matches in any single section of a reference file means you defaulted, not decided.** Discard and rebuild from the anchor in step 1. Don't patch the same draft with surface tweaks — a new icon, a bigger headline, a different shade of purple is not a revision, it's the same default wearing a costume.

Load only the file(s) the current task actually touches — a copy-only task doesn't need the DevOps reference open.

### 1d. Self-check

Before showing the output, run the six questions in `references/self-check-and-pushback.md`. Any "no" or "I don't know" answer means revise before showing it.

### 1e. Record — only for multi-piece work in the same session

Claude has no memory across separate conversations. A rule like "never converge on the same choices across generations" is only coherent *within one active session* — claiming it holds across sessions doesn't survive contact with how the model actually works, so don't imply otherwise to the user.


---
## Section 3 — Copywriting & Content Strategy (Content Developer perspective)

Use this section whenever writing, drafting, or revising prose of any kind for the user (marketing copy, landing pages, documentation, articles, essays, social posts, emails, or UI text).

### 3a. Sentence-shape and syntactic clichés

- **"In today's fast-paced/digital/ever-changing world..."** — any throat-clearing opener that establishes broad context instead of stating a concrete claim.
- **"Unlock/Unleash/Elevate/Supercharge/Empower your [noun]."** — verb-of-vague-empowerment + possessive + abstract noun. The single most over-trained grammatical slot in SaaS copy.
- **"Whether you're a [X] or a [Y], [Product] has you covered."** — the faux-inclusive opener trying to address every persona simultaneously.
- **"Say goodbye to [pain point]."** — written as if the pain point is a character leaving a party.
- **"Built for teams of every size"** or **"Built for [speed/scale/teams] at every level."** — the hedge that avoids defining a target user.
- **Rule-of-three adjective/phrase stacks** — triplets listed to make thin content sound comprehensive (e.g., "Fast. Reliable. Secure.", "Simple. Powerful. Flexible.", or "innovative, transformative, and groundbreaking").
- **Negative parallelism** — constructions like "It's not X, it's Y" ("It's not just a product launch, it's a movement").
- **Contrast-reframe statements** — overuse of "but" to manufacture false depth where no real contrast exists.
- **Em-dash sentences that restate the same claim twice** — "We don't just build software — we build the future of work." The second half adds no new information.
- **Sentence-initial conjunctions/transitions** — heavy, repeated use of "Additionally," "Moreover," "Furthermore," and "Consequently" to open sentences as transitional padding.
- **Bulleted feature lists with identical gerund starters** — "Streamline your workflows. Automate your pipelines. Optimize your delivery." Gerund + possessive noun, repeated for 6–8 bullets.
- **AI Storytelling Tropes / Narrative Arcs** — formulaic Problem-Agitation-Solution structures on every landing page section, or presenting basic feature releases as a dramatic "Hero's Journey."

### 3b. Structural and section content defaults

- **Generic page titles** like "Home | Product" or "About Us | Company" that communicate nothing specific to search engines or users.
- **Meta descriptions reading like brochures** rather than direct answers to user search intent.
- **"Our Story" / "About Us" LinkedIn-style summaries** — founded in [year], mission to disrupt, passionate team, world-class product.
- **Rhetorical question introductions** — opening blog posts or guides with questions the reader did not ask ("Ever wondered why [generic industry problem]?").
- **Vague noun section headers** ("Features," "Solutions," "Why Us," "Get Started") rather than explicit claims or promises.
- **Generic CTA button copy** ("Get Started," "Learn More," "Try Free") that fails to state the exact result of the action.
- **Filler FAQ content** — answering invented setup questions ("What is [Product]?", "How do I get started?") instead of real sales/objection call questions.
- **Unattributed social proof** ("Trusted by [N]+ teams", "Leading companies trust us") without verified client names or outcomes.
- **Formulaic document outlines** — ending articles with a generic "Challenges" or "Future Prospects" section that closes with a hollow call to action.

### 3c. Tone, voice, and editorializing defaults

- **Writing in a "professional" register that strips personality** — flat, inoffensive, interchangeable with any competitor.
- **Persona abstraction** — addressing the user as "you" while describing them as an abstract persona ("Teams like yours need...") rather than a person with a specific problem.
- **Empty buzzwords** — using "seamless," "intuitive," "powerful," "robust," "cutting-edge," or "best-in-class" without supporting evidence.
- **Active voice mimicry without subject clarity** — dangling phrases like "Enabling teams to ship faster" with no grammatical subject.
- **Puffery and inflated importance** — asserting significance without evidence and describing routine facts as "transformative" or part of a "broader movement."
- **Tailing clauses** — tacked-on present-participle phrases that add vague significance without information (e.g., "...reflecting the continued relevance of local traditions").
- **Weasel wording and vague attribution** — attributing opinions to "some critics say," "many believe," or "experts note" without naming sources.
- **Superficial editorializing** — inserting generic positive commentary or "sanding down" unusual facts into average claims.
- **Excessive hedging and qualifiers** that weaken claims unnecessarily.

### 3d. Lexical and rhythm tells

AI models default to predictable vocabulary and ultra-smooth cadence. Cut these on sight:
...
- **Overused Transitions:** moreover, furthermore, thus, consequently, in conclusion, "it's important to note/remember," "no discussion would be complete without."
- **Flawless but flat rhythm** — zero tangents, no conversational unevenness, and paragraphs of identical length.
- **Letter-template politeness in casual copy** — "I hope this message finds you well," "Thank you for your time and consideration."

### 3e. Formatting tics, stock phrases, and leftover AI artifacts

- **Excessive boldface and title case** in headers and list items.
- **Overused em dashes** with spaces around them (contrary to standard typographic rules) inserted where commas or colons belong.
- **Curly quotes or emojis in technical documentation headers**.
- **Malformed heading hierarchy** (e.g. jumping from H1 straight to H3).
- **Leftover conversational artifacts** — "I hope this helps!", "Of course!", "Let me know if you need anything else."
- **Unedited phrasal templates** — fill-in-the-blank instructions left inside final content.
- **Knowledge-cutoff and safety disclaimers** — unnecessary disclaimers ("It's important to consult a professional...") tacked onto non-risk topics.

### 3f. Hallucination-adjacent and promotional tells

- **Fabricated citations and metrics** — fake quotes, invented studies, or invalid source links attached to claims.
- **Promotional tone drift** — persistent shift toward advertisement language even when asked for neutral reporting.
- **The "active social media presence" tic** — noting that a subject "maintains an active social media presence" (a distinct LLM phrasing).
- **Romanticizing language** — undue emphasis on "cultural heritage," "enduring legacy," or "vibrant landscape" on mundane topics.
